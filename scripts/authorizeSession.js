...
class E2EECrypto {
  ...
  static _sha1Hash(data) {
    const hash = crypto.createHash('sha1');
    hash.update(data);
    return hash.digest();
  }

  static _getRandomBytes(length) {
    return crypto.randomBytes(length);
  }

  static _pkcs1PadType2(data, keySizeBytes) {
    const padded = new Uint8Array(keySizeBytes);
    padded[0] = 0x00;
    padded[1] = 0x02;
    const psLength = keySizeBytes - 3 - data.length;
    const ps = E2EECrypto._getRandomBytes(psLength);
    for (let i = 0; i < psLength; i++) {
      while (ps[i] === 0) ps[i] = E2EECrypto._getRandomBytes(1)[0];
      padded[i + 2] = ps[i];
    }
  ...
  encryptPassword(plainTextPassword) {
    ...
    const hashedTextBytes_ed = E2EECrypto._sha1Hash(textAsBytes);
    const randomBytes_bY = E2EECrypto._getRandomBytes(da);
    const component_cJ_bytes_dP = E2EECrypto._hexToBytes(this.cJ_internal_RandomNumber);
    ...
    const paddedMessageBigInt_m = E2EECrypto._pkcs1PadType2(concatenated_aY, this.keySizeBytes);

    const encryptedBigInt_c = E2EECrypto._modPow(paddedMessageBigInt_m, this.e_exponent, this.n_modulus);
...
async function callLogin(loginUrl, loginPayload, headers) {
  ...
  return {
    success: false, result: response.data, statusCode: response.statusCode, headers: response.headers,
  };
}
